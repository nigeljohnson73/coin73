...
function compressFile($fn, $elh = false) {
	global $cfiles, $ncfiles;
		global $compress_js;
	if ($elh) {
		if (!$compress_js || strtolower ( @ $_SERVER ['SERVER_NAME'] ) == "localhost") {
			$ncfiles [] = $fn;
		} else {
			$cfiles [] = $fn;
		}
...
$str = processSendableFile($str);
$ncstr = processSendableFile($ncstr);
...
if ($debug)
	echo '/* uncompressed: ' . number_format ( strlen ( $str . $ncstr ), 0 ) . ' bytes, ';
